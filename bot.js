  ...
  let prefix = await db.fetch(`prefix_${msg.guild.id}`) || ayarlar.prefix
    ...
    .setDescription('**:warning: | Lütfen öncelikle sesli bir kanala katılınız.**'));
     ...
     .setTitle(':warning: **| Hiç Bir Müzik Çalmamakta**'));                                              
		...
		if(args[0] > 10) return msg.channel.send(`Lütfen **1-10** arasında miktar giriniz.`)
		if (!msg.member.voiceChannel) if (!msg.member.voiceChannel) return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
    .setDescription(':warning: **| Lütfen öncelikle sesli bir kanala katılınız.**'));
		if (!serverQueue) return msg.channel.sendEmbed(new Discord.RichEmbed()
   ...
   .setTitle(`:warning: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    let index = 0;
		if (!serverQueue) return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setTitle(":warning: | **Sıra'da Müzik Bulunmamakta**")
    .setColor('RANDOM'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
...
function play(guild, song) {
  ...
  .setTitle("**Elvis| 🎙 Müzik Başladı**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
